...
To quantify this empirically, we computed both $\bar{\kappa}_h$ and $\bar{\kappa}_s$ from the \emph{same} centroid trajectories $\vect{C}(h)$ for all 200 structures (replication sanity check: $r(\bar{\kappa}_h,\ \overline{v_0}^{\mathrm{official}}) = 1.000$). The results are reported in \S\ref{sec:arclength_comparison}. In brief:
\begin{itemize}
    \item $r(\bar{v}_0, \bar{\kappa}_s) = -0.069$ ($p = 0.33$): the two metrics are statistically independent.
    \item Arc-length curvature shows no detectable association with Mamton structural roughness ($r = 0.011$, $p = 0.88$), whereas Vij exhibits strong coupling ($r = 0.870$, $p < 10^{-60}$).
\end{itemize}
The mean $ds/dh = 0.571$ indicates that the helical backbone is substantially tilted relative to the PCA axis, so $v_0(h)$ specifically captures \emph{transverse} bending relative to the global elongation direction rather than aggregate arc curvature. The mechanistic implication --- that Mamton responds to axis-relative lateral distortion and not to intrinsic path curvature --- is developed in \S\ref{sec:arclength_comparison}.
